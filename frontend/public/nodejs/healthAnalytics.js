...
const loadUserHealthContext = (db, email, callback) => {
  db.users.findOne({ email }, (userErr, userRow) => {
    if (userErr) {
      callback(userErr);
      return;
    }
    ...
    db.profiles.findOne({ user_id: userId }, (profileErr, profileRow) => {
      if (profileErr) {
        callback(profileErr);
        return;
      }

      db.recipe_records.find({ user_id: userId }).sort({ date: -1, created_at: -1 }).exec((recipeErr, recipeRows) => {
        if (recipeErr) {
          callback(recipeErr);
          return;
        }

        db.user_goals.find({ user_id: userId }).sort({ created_at: 1 }).exec((goalErr, goalRows) => {
          if (goalErr) {
            callback(goalErr);
            return;
          }

          db.user_preferences.find({ user_id: userId }).sort({ created_at: 1 }).exec((prefErr, prefRows) => {
            if (prefErr) {
              callback(prefErr);
              return;
            }
            ...
            callback(null, {
              ...
              profile: {
                ...
                dietStyle: profileRow?.dietStyle || '',
                mood: profileRow?.mood || '',
